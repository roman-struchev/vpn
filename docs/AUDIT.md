...
| C16 | 🔴 | Desktop v0.1.23: проба живости ждала тело trace-страницы, а по HTTP `1.1.1.1` отвечает редиректом 301 — Desktop не мог подключиться вообще | `tunnelProbe.ts` | ✅ v0.1.24 (признак — заголовок `CF-RAY`) |
| C17 | 🟠 | Android объявлял CONNECTED сразу после `runXray`, до проверки — ложное «Защищено» после каждой попытки | `XrayVpnService` | ✅ |
| C18 | 🟠 | Android: проба живости переиспользовала keep-alive соединение, открытое до отзыва ключа, и продолжала проходить | `XrayVpnService` | ✅ |
| C19 | 🟡 | «Нет подписки» по устаревшему профилю сразу после покупки тарифа | `ConnectFragment` | ✅ |
| C20 | 🟢 | Debug-сборка предлагала «обновиться» до релизного APK с другой подписью | `AppUpdateManager` | ✅ |

C16–C18 найдены прогоном на реальной ноде, а не юнит-тестами:
* Android — `android/scripts/real-tunnel-e2e.sh` (эмулятор + локальный agent + настоящий xray-core; `TunnelFlowTest`: подключение, трафик, стабильность, отзыв ключа с самовосстановлением, отключение).
* Desktop — `desktop/test/realTunnel.integration.test.ts` (запуск с `REAL_NODE_API`, нода с `PUBLIC_IP=127.0.0.1`).

Там же подтверждено, что `ProtectedSocketFactory` (`bind(null)` + `protect()`) работает на реальном Android: после отзыва сервис перерегистрировал устройство запросом, который мимо туннеля не прошёл бы. Привязка к исходной сети (`Network.bindSocket`) для VPN-приложения запрещена (EPERM).
