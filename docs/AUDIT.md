...
| **1.2** | 🟡 Средний | Асимметрия срока действия триала (30 дней vs 3 дня) | `BillingService.java:269-276` | ✅ Везде 1 ГБ без срока |
...
### P2P-выход, проверка на двух эмуляторах (23.09.2026)

Режим P2P-выхода до этого ни разу не проверялся вживую, и не работал вовсе. Цепочка найденного:

| № | Уровень | Проблема | Где | Статус |
|---|:---:|---|---|:---:|
| P1 | 🔴 | Сервер писал сигналы в gRPC-поток пира одновременно из разных HTTP-потоков; `StreamObserver` не потокобезопасен — поток пира рвался на первом же offer | `AgentStreamServiceImpl` | ✅ сериализация записи |
| P2 | 🔴 | Android: собственный WebRTC приложения попадал в свой же TUN (UDP блокировался, STUN шёл через ещё не установленного пира) | `XrayVpnService` | ✅ на P2P-путях приложение вне TUN, проба через `probe-in` |
| P3 | 🔴 | Android слал данные в DataChannel как текст — desktop/Linux-пир (node-datachannel) получал UTF-8-строку, бинарные байты терялись, TLS не проходил | `P2pClientSession`, `P2pWebRtcSession` | ✅ `binary=true` |
| P4 | 🟠 | ICE-кандидаты, обогнавшие offer (или пришедшие до завершения `setRemoteDescription`), отбрасывались — на всех трёх пирах и на Android-клиенте | agent, desktop, Android | ✅ буферизация |
| P5 | 🟠 | Android-пир после обрыва gRPC-потока не переподключался; запись в поток тоже не была сериализована | `P2pRelayAgent` | ✅ |
| P6 | 🟠 | Учёт: сессия учитывалась один раз по первой паре отчётов, хотя обе стороны шлют накопительные итоги — длинная сессия почти не списывалась с квоты и почти не оплачивалась пиру; трафик выхода не списывался, если у пира нет владельца или он упёрся в дневной лимит | `P2pRelayAccountingService` | ✅ учёт по приросту, списание всегда |
| P7 | 🟡 | Список регионов не перезагружался после неудачной загрузки (после перехода на show/hide вкладок) | `ConnectFragment` | ✅ |

Итог: Android-клиент подключается через пира (хост-агент в роли пира), проба туннеля проходит через пира, трафик списывается с квоты клиента.

Два Android-эмулятора друг с другом соединиться не могут: оба за QEMU-NAT (SLIRP), который не принимает входящий UDP, а TURN в дизайне сознательно не используется (см. `P2P_RELAY.md`). Сигналинг и ICE-проверки между ними идут, падают только проверки связности — это ограничение стенда, между реальными устройствами в обычных NAT это не так.
