...
2. ~~**Телеметрия не репортится при успешном подключении**~~ — **сделано**:
   Android/Desktop клиенты теперь репортят телеметрию и на успешный
   `TUNNEL_UP` (с реально замеренным `connectTimeMs`, не только на сбой).
   `ConnTelemetryRepository.aggregateByOperatorAndRegion` и
   `AdminController.getDashboardMetrics` различают успех/неудачу:
   `successCount`, `failureReportCount`, `failureRatePercent` (реальная доля
   отказов, раньше знаменателя не было вообще) и `avgConnectTimeMs` на бакет
   `оператор × регион × транспорт`. Server +2 теста (79 всего).
