# Архитектура и системный дизайн Aura VPN

Документ описывает архитектуру, технические решения, протоколы, модель данных и принципы работы сервиса Aura VPN. Служит постоянным источником правды об устройстве системы.

---

## 1. Концепция и цели сервиса

**Aura VPN** — высокоустойчивый к цензуре VPN-сервис, спроектированный для надёжной работы в условиях активного противодействия DPI (включая ТСПУ в РФ, GFW в Китае, цензуру в Иране).
...
* **Агент всегда звонит на сервер сам**: на ноде закрыты все входящие порты управления. Нода регистрируется по bootstrap-токену (может использоваться многократно в пределах срока действия) и поднимает постоянный исходящий mTLS gRPC стрим к серверу.
...
### 6.1.1. Сессии клиентов (JWT)
* Срок жизни токена — `security.jwt.expiration-hours` (720 ч). Клиенты (Android, Desktop, Web) продлевают его сами через `POST /api/v1/auth/refresh`, когда остаётся меньше 7 дней (не чаще раза в час). Заблокированный аккаунт продлить нельзя.
* **401** — сессии нет (токен отсутствует, истёк, отозван, аккаунт заблокирован), тело `{"error":"Unauthorized"}`. **403** — сессия есть, но действие запрещено (роль, чужой ресурс, P2P-ограничения). Error-dispatch `/error` разрешён отдельно, иначе настоящий 403 превращался бы в 401 при повторной обработке.
* На 401 гостевой (device-trial) аккаунт молча перелогинивается через `/auth/device` и повторяет запрос. Зарегистрированный пользователь попадает на экран входа с пояснением «Сессия истекла». Создавать гостя вместо зарегистрированного пользователя клиенты не должны: так его тариф «спрятался» бы за новым триалом.
...
* Селектор регионов с индикацией задержки и загруженности. Пинг берёт хост ноды из уже полученных ссылок и не измеряется при поднятом туннеле.
* **Надёжность туннеля:** служебные Java-сокеты сервиса (API при переподключении, сигналинг P2P, проверка блокировки оператором) идут через `ProtectedSocketFactory` мимо собственного TUN. Живость проверяется каждые 30 с HTTPS-запросом к `https://1.1.1.1/cdn-cgi/trace` через туннель: TCP-connect не годится, его завершает сам TUN-стек xray. В `OPERATOR_BLOCKED`/`ERROR` сервис закрывает TUN и останавливает xray. После полного круга неудач профиль и ссылки запрашиваются заново. Причина ошибки (`NO_SUBSCRIPTION`, `SESSION_EXPIRED`, `NO_SERVERS`, `NETWORK`) показывается в UI.
...
* **Надёжность туннеля:** «Защищено» выставляется только после того, как HTTP-запрос к `http://1.1.1.1/cdn-cgi/trace` прошёл через отдельный inbound `probe-in` (порт 10810, всегда в туннель при любом RU-режиме). Системный прокси включается после этой проверки. Далее — проверка каждые 30 с. `XrayProcess.stop()` ждёт фактического выхода процесса. Смена региона или режима не снимает системный прокси. В тупиковых состояниях прокси снимается, и машина не остаётся без интернета.
* Ограничение прокси-режима: приложения, которые игнорируют системный прокси (часть CLI, игры, UDP/WebRTC), идут напрямую, kill switch нет. TUN-режим — в `TODO.md`.
* **Обновления:** проверка при запуске и раз в 4 часа по GitHub Releases, баннер в интерфейсе. На Windows electron-updater ставит обновление при перезапуске. На macOS его установщик (Squirrel.Mac) не работает с неподписанным приложением, поэтому по кнопке запускается вшитый `scripts/install-mac.sh`: он подменяет бандл, и приложение перезапускается само (схема aura-pad). Сборки до v0.1.25 включительно так не умеют — их нужно один раз обновить вручную.
...
|---|---|---|---|---|---|
| **Пробный (Trial)** | 0 | — | 1 ГБ (без срока) | 1 | Общий пул `trial`, ограничение канала на ноде |
| **Basic** | $1 | $10 | 20 ГБ | 2 | Обычный пул `paid` |
| **Pro** | $2 | $20 | 100 ГБ | 5 | Приоритетный пул `paid`, скорость без ограничений |

* **1 ГБ без ограничения по времени — это пробный период, а не вечный бесплатный тариф**: он кончается вместе с трафиком. Одинаков для всех способов входа (приложение, сайт, бот, Google). Выдаётся строго один раз на аккаунт/устройство. Повторная активация отклоняется сервером.
...
* [`AUDIT.md`](AUDIT.md) — Отчёт технического аудита: сервер и клиенты (22.09.2026), статус исправлений.
