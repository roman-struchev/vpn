...
Оркестрация парка серверов Aura VPN построена по схеме **обратного управления (Inverted Control Plane)**: центральный сервер никогда не инициирует сетевые соединения к нодам. Напротив, нодовый агент сам подключается к бэкенду.
...
1. Администратор генерирует bootstrap-токен (действует многократно до истечения срока) через админ-панель или API:
