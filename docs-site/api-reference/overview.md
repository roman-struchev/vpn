...
Документация описывает все программные интерфейсы центрального сервера NextGen VPN (`server/`): публичный REST API, защищенный клиентский API, панель управления администратора, вебхуки Telegram и бидирекциональный gRPC протокол управления нодами.
