...
│   ├── POST /users/{id}/subscription/temporary-tariff (Временный тариф на N дней)
│   ├── POST /users/{id}/subscription/temporary-tariff/cancel (Отмена временного тарифа)
│   ├── GET  /nodes                  (Мониторинг нод, CPU, RAM, пинг, трафик, статус)
│   ├── POST /nodes/bootstrap-token  (Выпуск токена регистрации ноды, многоразовый до истечения)
